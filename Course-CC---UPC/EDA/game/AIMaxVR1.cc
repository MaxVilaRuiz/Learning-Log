...
struct PLAYER_NAME : public Player {
  ...
  /**
   * @brief Returns whether a cell is occupied by a soldier or a professor of my team 
   */
  bool teammate_blocking(Pos np, int my_player) {
    Cell c = cell(np);
    if (c.id == -1) return false;
    Unit u = unit(c.id);
    return u.player == my_player;
  }
  ...
  void play_professor(int prof_id, int my_player) {
    ...
    // Try to get high value money first
  ...
  void play_soldier(int sid, int my_player) {
    ...
    if (s.poison > 0) {
      ...
      if (found) {
        Pos np = s.pos + dir;
        if (!teammate_blocking(np, my_player)) move(s.id, dir);
        else random_move(s, my_player);
        return;
      }
    }
