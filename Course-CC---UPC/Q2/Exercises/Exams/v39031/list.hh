...
class List {
 ...
 public:
	...
	template <typename U>
	friend istream& operator>>(istream& is, List<U>& l);
 ...
 public:
	 ...
	 *
	 * @post 'it' continua apuntando al mismo elemento, el cual ha sido movido al
	 *       inicio de la llista. No se ha reservado ni liberado memoria. En el caso
	 *       en que el elemento apuntado por 'it' ya fuese el primero, nada ha 
     *       cambiado.
	 */
	void moveToBeginning(iterator& it) {
		Item* paux = it.pitem;
		if (paux != iteminf.next) {
			paux->prev->next = paux->next;
			paux->next->prev = paux->prev;

			paux->prev = &iteminf;
			paux->next = iteminf.next;
			iteminf.next->prev = paux;
			iteminf.next = paux;
		}
	}
