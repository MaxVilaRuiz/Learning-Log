...
public:
  ...
  void swapFirstLast() {
    if (_size == 2) {
      ...
      itemsup.prev = paux;
      iteminf.next->next = itemsup.prev;
      paux->next = &itemsup;
      paux->prev = iteminf.next;
    }
    else if (2 < _size) {
      Item* paux = iteminf.next;
      iteminf.next = itemsup.prev;
      iteminf.next->next = paux->next;
      paux->next->prev = iteminf.next;
      paux->prev = iteminf.next->prev;
      iteminf.next->prev->next = paux;
      iteminf.next->prev = &iteminf;
      paux->next = &itemsup;
      itemsup.prev = paux;
    }
