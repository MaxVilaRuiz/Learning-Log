...
public:
  ...
  // Pre: 0 <= n < size()
  // Post: S'ha eliminat l'element que estava a la posició 'n' (començant en 0).
  //       Els elements posteriors s'han desplaçat una posició cap endavant.
  //       No es fa servir removeItem, però sí que s'enllacen i s'alliberen manualment els nodes.
  //       Si la posició no és vàlida, el programa finalitza amb error.
  void remove_at(int n) {
    if (n < 0 or n >= _size) {
      cerr << "Error: remove_at amb posició fora de rang" << endl;
      exit(1);
    }
