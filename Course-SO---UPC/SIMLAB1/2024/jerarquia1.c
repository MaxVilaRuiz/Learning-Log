...
void Usage() {
    ...
    sprintf(buffer, "To create a process child for each file entered in the input and know the pid, the corresponding file and the exit status of each child\n");
    write(1, buffer, strlen(buffer));
}

int main(int argc, char *argv[]) {
    char buffer[2000];
    int pid_childs[argc-1];
