...
public:
  ...
  /*
   * Ejercicio 4: Comprueba si el árbol actual y otro 
   *   árbol a son iguales (estructura y contenido).
   */
  bool iguals_nodes(node_arbre* a1, node_arbre* a2) const {
    if (a1 == nullptr and a2 == nullptr) return true;
    if (a1 == nullptr or a2 == nullptr) return false;
    if (a1->info != a2->info) return false;
    return iguals_nodes(a1->segE, a2->segE) and iguals_nodes(a1->segD, a2->segD);
  }
  
  bool iguals(const Arbre<T>& a) const {
    return iguals_nodes(primer_node, a.primer_node);
  }
