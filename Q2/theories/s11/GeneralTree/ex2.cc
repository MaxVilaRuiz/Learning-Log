...
template <class T> class ArbreNari { 
   ...
   *   Implementa int altura() const, que devuelva la altura del árbol. 
   *   Un árbol vacío tiene altura 0. Un nodo tiene altura 1 + máximo de 
   *   las alturas de sus hijos.
   */
  int altura_node(node_arbreNari* n) const {
    if (n == NULL) return 0;
    int hmax = 0;
    for (int i = 0; i < int(n->seg.size()); ++i) {
      hmax = max(hmax, altura_node(n->seg[i]));
    }
    return 1 + hmax;
  }

  int altura() const {
    return altura_node(primer_node);
  }
